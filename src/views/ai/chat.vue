...
const session = ref<any>({
    stream: true, model: 'gpt-4', messages: [],
    timeline: {}
})
...
function loadMessages() {
    ...
    axios.admin({
        url: '/api/ai/session/message/list',
        method: 'post',
        data: {
        ...
        if (res.success) {
            session.value.messages = [...session.value.messages, ...(res.body || [])]
...
function timelineProcess(list: any) {
    ...
    if (!session.value.timeline || !session.value.timeline.length) {
...
function sendMessage() {
    ...
    axios.admin({
        url: '/api/ai/session/message/send',
        method: 'post',
        data: {
